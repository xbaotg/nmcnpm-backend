...
from utils import valid_add_match, valid_update_match, is_int, convert_from_attr, is_admin
...
async def update_match(db: db_deps, current_user : CurrentUser, update: MatchUpdate, id : int):
    is_admin(db, current_user)
    ...
    try:
        ...
        if (update.goal1 != -1 and update.goal2 != -1):
            ...
            if not (today >= start_time):
                raise HTTPException(status_code = 400, detail = f"Can't update result for unfinished matches !")
...
async def update_result(db: db_deps, current_user: CurrentUser, id: int, goal1: int, goal2: int):
    is_admin(db, current_user)
    ...
    if (today < target.start):
        raise HTTPException(status_code=400, detail = "Can't update result for unfinished matches!")
...
async def permanently_delete_match(db: db_deps, current_user : CurrentUser, id : int):
    ...
    if not target:
        return {"message": "Can't find any matches, make sure to temporarily delete first "}
