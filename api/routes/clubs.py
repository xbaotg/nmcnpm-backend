...
async def search_club_by_manager_id(db: db_deps, manager_search_id: int):
    ...
    target = db.query(Users).filter(Users.show == True, Users.user_id == manager_search_id, Users.role == "manager").first()
    if target is None:
    ...
    db_clubs = db.query(Clubs).filter(Clubs.show == True, Clubs.manager == manager_search_id).all()
    if not db_clubs:
        raise HTTPException(status_code=204, detail="This manager doesn't have any clubs")
